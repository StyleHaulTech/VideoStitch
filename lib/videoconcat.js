...
module.exports = function (spec) {
  ...
  function concatClips(args) {
    ...
    return new Promise((resolve, reject) => {
      let child = shelljs.exec(`${ffmpeg.path} -f concat -safe 0 -protocol_whitelist file,http,https,tcp,tls,crypto -i ${args.fileList} -c copy ${outputFileName}`, { async: true, silent: spec.silent });
